...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.7/contracts/token/ERC20/IERC20.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.7/contracts/token/ERC20/extensions/draft-IERC20Permit.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.7/contracts/token/ERC20/utils/SafeERC20.sol
...
contract StakingContract is Ownable {
    ...
    uint256 public totalStakedOneWeek;
    uint256 public totalStakedOneMonth;
    uint256 public totalStakedThreeMonths;
    ...
    function unstake(uint256 stakeIndex) external {
        ...
        // Atualizando os totais staked de acordo com a duração
        if (userStake.duration == 1 weeks) {
            totalStakedOneWeek -= userStake.amount;
        } else if (userStake.duration == 4 weeks) {
            totalStakedOneMonth -= userStake.amount;
        } else if (userStake.duration == 12 weeks) {
            totalStakedThreeMonths -= userStake.amount;
        }
    ...
    function calculateEstimatedReward(uint256 amount, uint256 duration) public view returns (uint256) {
        // Calculates the reward based on the APR for the given amount and duration
        uint256 aprPerSecond = apr * 1e16 / (365 days);
        uint256 reward = (amount * duration * aprPerSecond) / 1e18;
        
        // Assuming all Ether in the contract is distributed proportionally
        uint256 totalEth = address(this).balance;
        uint256 totalTokens = stakingToken.balanceOf(address(this));
    ...
    function getTotalStakedOneWeek() external view returns (uint256) {
        return totalStakedOneWeek;
    }

    function getTotalStakedOneMonth() external view returns (uint256) {
        return totalStakedOneMonth;
    }

    function getTotalStakedThreeMonths() external view returns (uint256) {
        return totalStakedThreeMonths;
    }
    ...
    function calculateEstimatedRewardOneWeek(uint256 amount) external view returns (uint256) {
        return calculateEstimatedReward(amount, 1 weeks);
    }

    function calculateEstimatedRewardOneMonth(uint256 amount) external view returns (uint256) {
        return calculateEstimatedReward(amount, 4 weeks);
    }

    function calculateEstimatedRewardThreeMonths(uint256 amount) external view returns (uint256) {
        return calculateEstimatedReward(amount, 12 weeks);
    }
